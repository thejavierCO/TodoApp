class Tododb extends EventTarget{
    constructor(base){
        super();
        this._db = [];
        switch(typeof base){
        ...
        try{
            switch(typeof base){
                ...
                case "object":
                    ...
                    break;
                default:
                    this.alert({in:"insert data in db",data:{base,isType:typeof base}})
            }
        }catch(err){
    ...
    update(id,data){
        if(typeof data === "object"){
            this.db = this.db.map((e,i)=>i==id?((a)=>{
...
// let db = new Tododb(localStorage.getItem("db")||localStorage.setItem("db","[]"))
// .on("error",({detail})=>{
//     console.log(detail)
// })
// .on("update",({target,detail})=>{
//     // console.log(target.firtsItem,detail)
...
// setInterval(()=>{
//     db.push({id:Math.round(Math.random()*10)})
//     if(db.length==2)db.firtsItem = {id:12839};
//     else if(db.length==3)db.lastItem = {id:Math.round(Math.random()*10)};
// },1500)