...
let api = new Tododb(
  ...
  .on('update', ({ target: db }) => {
    ...
    db.db.map((e, i) => {
      ...
      modelI
        .find('button#itemDelete')
        .click(({ target }) => api.del($(target).parent().attr('itemid')))
      modelI
        .find('button#itemEdit')
        .click(({ target }) =>
          id
            .val($(target).parent().attr('itemid'))
            .get(0)
            .dispatchEvent(new Event('change')),
        )
      modelI
        .find('button#itemComplete')
        .click(({ target }) =>
